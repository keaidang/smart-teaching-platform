...
// 课前检测题面（项目一·任务2 传感与视觉数据采集，工单 SQ-2026-001；6 题合计 100 分）
...
// 课后习题题面（传感与视觉数据采集 · 对应工单 SQ-2026-001）
export const exercises: Exercise[] = [
  ...
  { id: "EX5", title: "质量筛选时某张样本的质量分只有 0.42（工单要求 ≥0.5 才能被底库采用），正确的处理方式是？", options: ["判定为不合格样本，不进入底库，重新采集该样本", "只要人脸完整清晰就可以直接入库", "手动把质量分改成 0.5 以上再入库", "交给特征提取环节，模型会自动纠错"] },
  { id: "EX6", title: "为了让模型适应不同角度的人脸，采集时同一名被采集人至少需要采集哪些样本？", options: ["1 张正脸就足够", "2 张：正脸 + 左侧脸", "至少 3 张不同姿态：正脸 + 左侧脸 + 右侧脸", "越多越好，直接用视频逐帧截取，不用筛选"] },
  { id: "EX7", title: "按照合规要求，采集人脸数据前应做的第一步是？", options: ["直接开始采集，事后补签同意即可", "先向被采集人说明数据用途与处理方式，签署知情同意卡后再采集", "被采集人口头答应就行，不用留下记录", "把同意书扫描件和原始照片一起存入底库"] },
  { id: "EX8", title: "最终交付打包时，符合工单提交要求的做法是？", options: ["把全部原始人脸照片和特征数据一起压缩上传", "按 SQ-2026-001_第X组.zip 命名打包，包含特征数据集、metadata.csv、datacard.md 与合规记录，不含原始图像", "只交特征数据集，索引表、数据卡和合规记录可以不交", "用聊天工具把文件逐个发给老师，不需要打包命名"] },
];
